...
namespace ExplorerTreeView.Controls
{
    ...
    public class ExplorerTreeView : TreeView
    {
        ...
        public void InitExplorer2()
        {
            Items.Clear();
            //DirectoryInfo di = new DirectoryInfo(@"I:\RocketLauncher\Media\Artwork\Amstrad CPC\2 Player Super League (Europe)");
            //Items.Add(GenerateDirectoryNode(di));
        }

        public void AddSelectedFoldersFromHyperSpin(DirectoryInfo di)
        {
            Items.Clear();
            Items.Add(GenerateDirectoryNode(di));
        }
